    ...
    function() {
        ...
        jQuery("#sendPushNotification").click(function(e){
          ...
          //  var media_id =  jQuery(parent).find("input[name='mobappSettings[opt-push-media][id]']").val();
           ...
           var action_value;
           if(action=='go_to_url' || action=='open_webview')
               action_value =  jQuery(parent).find(".wooAppPushNotify_Action > .widget-action-value-field").find("input,select").val();
           else if(action!='')
               action_value =  jQuery(parent).find(".wooAppPushNotify_Action > .widget-action-value-field").find("input,select").select2('val');
           else
               action_value = '';
