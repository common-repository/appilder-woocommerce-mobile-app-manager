...
class WOOAPP_API_Authentication
{
    ...
    public function __construct()
    {
        ...
        add_filter('woocommerce_mobapp_api_set_auth_key', array($this, 'set_user_auth_key'),0);
    ...
    public function authenticate($user)
    {
        ...
        try {
            ...
            if (in_array(getapi()->server->current_route,
                array('/push/gcm/register', '/', '/meta', '/products', '/products/product', '/products/filters', '/products/reviews', '/menu', '/user_register', '/inAppPages', '/inAppPages/page' , '/social-login'
                )))
    ...
    private function perform_api_key_check()
    {
        ...
        if (!empty($params['api_key'])) {
            ...
            throw new Exception(__('API Key is missing', 'woocommerce_mobapp'), 401);
        }

        if (!empty($params['api_secret'])) {
            $api_secret = $params['api_secret'];
        } else {
            throw new Exception(__('API Secret is missing', 'woocommerce_mobapp'), 401);
        }
    ...
    private function password_auth()
    {
        ...
        if (!empty($params['user_username'])) {
            ...
            throw new Exception(__('Username is missing', 'woocommerce_mobapp'), 401);
        }
        if (!(empty($params['user_password']) && empty($params['user_pass']))) {
            ...
            throw new Exception(__('Password is missing', 'woocommerce_mobapp'), 401);
        }
        ...
        if ($user && wp_check_password($user_password, $user->data->user_pass, $user->ID)) {
            ...
            throw new Exception(__('Invalid username/password', 'woocommerce_mobapp'), 401);
    }
    ...
    private function user_auth()
    {
        ...
        if (!empty($params['user_auth_key'])) {
            ...
            throw new Exception(__('User auth key is missing', 'woocommerce_mobapp'), 401);
        }
        if (!empty($params['user_auth_secret'])) {
            $user_auth_password = $params['user_auth_secret'];
        } else {
            throw new Exception(__('User auth password is missing', 'woocommerce_mobapp'), 401);
        }
