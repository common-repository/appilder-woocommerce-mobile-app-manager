...
class WOOAPP_API_Cart extends WOOAPP_API_Resource {
    ...
    public function get_cart_meta(){
        ...
        return apply_filters("appilder_woocommerce_cart_meta_response",$return);
    }
    ...
    public function get_cart_items($shipping_methods=array()){
        ...
        wc_clear_notices();
        return apply_filters("appilder_woocommerce_cart_response",$return);
    }
    ...
    public function get_wp_notices_error(){
        ...
        if(!empty($notices['error'])){
            ...
            foreach($notices['error'] as $key=>$error){
                $return = WOOAPP_API_Error::setError($return,'cart_add_error_'.$key, html_entity_decode($error));
            }
            wc_clear_notices();
            return $return;
        }else{
    ...
    public function add_to_cart($product_id,$quantity,$variation_id=null,$variation=''){
        $this->cart();
        $_POST['add-to-cart'] = $_REQUEST['add-to-cart'] = $product_id;
        add_filter('option_woocommerce_cart_redirect_after_add' , array(self::name(),'returnNo'),9999);
        add_filter('woocommerce_add_to_cart_redirect' , array(self::name(),'returnFalse'),9999);
    ...
    public static function returnNo ($value){
        return 'no';
    }
    ...
    public function update_quantity($key, $quantity,$refresh_totals = true,$return_item = true ) {
        $product = $this->cart()->get_cart_item($key);
        if (!$product['data']->has_enough_stock( $product['quantity'] + $quantity ) ) {
            return  WOOAPP_API_Error::setError(false,"cannot_add",sprintf( __( 'You cannot add that amount to the cart &mdash; we have %s in stock and you already have %s in your cart.', 'woocommerce' ), $product['data']->get_stock_quantity(),$product['quantity']));
        }
    ...
    public function calculate_shipping($calc_shipping_country=false,$calc_shipping_state='',$has_to_return = true){
        $return = array();
        ...
        try {
            ...
            */
            ...
            if(apply_filters( 'woocommerce_shipping_calculator_enable_postcode', true ) && isset($_POST['calc_shipping_postcode']) && !empty($_POST['calc_shipping_postcode'])  &&  $_POST['calc_shipping_postcode'] != 'null' )
                 $data['calc_shipping_postcode'] = wc_clean($_POST['calc_shipping_postcode']);
             else
                 $data['calc_shipping_postcode'] = '';

            if(apply_filters( 'woocommerce_shipping_calculator_enable_city', true)  && isset($_POST['calc_shipping_city'])  && !empty($_POST['calc_shipping_city']) &&  $_POST['calc_shipping_city'] != 'null'  )
                 $data['calc_shipping_city'] = wc_clean($_POST['calc_shipping_city']);
             else
                 $data['calc_shipping_city'] = '';
