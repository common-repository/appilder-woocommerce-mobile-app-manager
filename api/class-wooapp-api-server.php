...
class WOOAPP_API_Server  {
    ...
    public function __construct( $path ) {
		...
		if ( $this->is_json_request() )
			$handler_class = 'WOOAPP_API_JSON_Handler';

		elseif ( $this->is_xml_request() )
			$handler_class = 'WOOAPP_API_XML_Handler';

		else
			$handler_class = apply_filters( 'woocommerce_mobapp_api_default_response_handler', 'WOOAPP_API_JSON_Handler', $this->path, $this );
	 ...
	 *
	 * @since 2.1
	 * @return WP_User|WOOAPP_API_Error WP_User object indicates successful login, WOOAPP_API_Error indicates unsuccessful login
	 */
	public function check_authentication() {

		// allow plugins to remove default authentication or add their own authentication
		$user = apply_filters( 'woocommerce_mobapp_api_check_authentication', null, $this );

		// API requests run under the context of the authenticated user
		if ( is_a( $user, 'WP_User' ) )
        {
            wp_set_current_user( $user->ID );
            getapi()->override_session();
        }
		// WOOAPP_API_Errors are handled in serve_request()
		//elseif (is_wooapp_api_error( $user ) )
		//	$user = WOOAPP_API_Error::setError($user,'woocommerce_mobapp_api_authentication_error', __( 'Invalid authentication method', 'woocommerce_mobapp' ), array( 'code' => 500 ) );
	 ...
	 *
	 * @since 2.1
	 * @uses WOOAPP_API_Server::dispatch()
	 */
	public function serve_request() {

		do_action( 'woocommerce_mobapp_api_server_before_serve', $this );

		$this->header( 'Content-Type', $this->handler->get_content_type(), true );

		// the API is enabled by default
		if ( ! apply_filters( 'woocommerce_mobapp_api_enabled', true, $this ) || ( 'no' === get_option( 'woocommerce_mobapp_api_enabled' ) ) ) {

			$this->send_status( 404 );

			echo $this->handler->generate_response( array( 'errors' => array( 'code' => 'woocommerce_mobapp_api_disabled', 'message' => 'The woocommerce_mobapp API is disabled on this site' ) ) );
		...
		$served = apply_filters( 'woocommerce_mobapp_api_serve_request', false, $result, $this );
	...
	public function get_routes() {
		...
		$endpoints = apply_filters( 'woocommerce_mobapp_api_endpoints', $endpoints );
	 ...
	 *
	 * @since 2.1
	 * @return mixed The value returned by the callback, or a WOOAPP_API_Error instance
	 */
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default:
				return new WOOAPP_API_Error( 'woocommerce_mobapp_api_unsupported_method', __( 'Unsupported request method', 'woocommerce_mobapp' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) )
					return new WOOAPP_API_Error( 'woocommerce_mobapp_api_invalid_handler', __( 'The handler for the route is invalid', 'woocommerce_mobapp' ), array( 'status' => 500 ) );
				...
				$args = apply_filters( 'woocommerce_mobapp_api_dispatch_args', $args, $callback );

				// Allow plugins to halt the request via this filter
				if ( is_wooapp_api_error( $args ) ) {
					return $args;
				}

				$params = $this->sort_callback_params( $callback, $args );
				if ( is_wooapp_api_error( $params ) )
					return $params;
		...
		return new WOOAPP_API_Error( 'woocommerce_mobapp_api_no_route', __( 'No route was found matching the URL and request method', 'woocommerce_mobapp' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			...
			elseif ( $param->isDefaultValueAvailable() ) {
				...
				return new WOOAPP_API_Error( 'woocommerce_mobapp_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'woocommerce_mobapp' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
	...
	public function get_index() {
		// General site data
        $WOOAPP_version  = get_plugin_data(__DIR__."/../plugin.php" );
        $WOOAPP_version = $WOOAPP_version['Version'];

		$available = array( 'store' => array(
			'name'        => get_option( 'blogname' ),
			'description' => get_option( 'blogdescription' ),
			'URL'         => get_option( 'siteurl' ),
			'WOOAPP_version'  =>    $WOOAPP_version,
			...
			foreach ( self::$method_map as $name => $bitmask ) {
				foreach ( $callbacks as $callback ) {
					...
					if ( strpos( $route, '<' ) === false ) {
						$data['meta'] = array(
							'self' => get_woocommerce_mobapp_api_url( $route ),
						);
					}
				}
			}
			$available['store']['routes'][ $route ] = apply_filters( 'woocommerce_mobapp_api_endpoints_description', $data );
		}
		return apply_filters( 'woocommerce_mobapp_api_index', $available );
	}
	...
	public function add_pagination_headers( $query ) {
		...
		if ( ! $single ) {
			...
			if ( $page > 1 ) {
                ...
                $this->pagination['pagination']['first'] =  1 ;
                $this->pagination['pagination']['prev'] = $page -1;
			}

			// next
			if ( $next_page <= $total_pages ) {
				$this->link_header( 'next', $this->get_paginated_url( $next_page ) );
                $this->pagination['pagination']['next'] = $next_page;
			}

			// last
			if ( $page != $total_pages ){
				$this->link_header( 'last', $this->get_paginated_url( $total_pages ) );
                $this->pagination['pagination']['last'] = $total_pages;
            }
        }

		$this->header( 'X-WC-Total', $total );
		$this->header( 'X-WC-TotalPages', $total_pages );
        $this->pagination['pagination']['total'] = $total;
        $this->pagination['pagination']['totalPages'] = $total_pages;

        add_filter( 'apply_pagination',array($this,'apply_pagination_fun'),1,1 );
        do_action( 'woocommerce_mobapp_api_pagination_headers', $this, $query );
	}
    public function apply_pagination_fun($data){
        if(is_array($this->pagination)){
            return array_merge($data,$this->pagination);
        }
        return $data;
    }
